...
export default function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("http://localhost:3000/api/items/Itcreate", {
        method: "POST",
        headers: {
...
<div>
      ...
      <h1 className="text-center text-4xl my-7 font-extralight text-black">Create Items</h1>
        ...
        {[0, 1, 2].map((index) => (
          ...
          >
            <input
              ...
              className="border border-green-700 bg-white rounded-md py-2 px-4 focus:outline-none focus:border-green-700"
            />
...
{formData[`image${index + 1}`] && (
            ...
            <button
              type="button"
              className="w-40 h-10 rounded-lg bg-green-700 text-white hover:opacity-90"
              size="sm"
        ...
        <div className="flex flex-col gap-4 sm:flex-row justify-between">
          <input
            className=" bg-slate-200  border border-slate-100 p-3 rounded-lg w-[460px] h-11"
            type="text"
        ...
        <button
          type="submit"
          className="bg-green-700  border border-white text-white p-3 rounded-lg w-[460px] h-11 hover:opacity-90"
        >
          Add
        </button>
